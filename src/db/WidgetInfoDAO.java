...
public class WidgetInfoDAO extends BaseDAO{
	public int insertWidgetInfo(int userCode, String widgetId, int widgetType, double xPos, double yPos, double xSize, double ySize){
		int randomNum = -1;
		int insertRowCnt = 0;
	...
	public int insertPictureInfo(int userCode, int widgetCode, int pictureCode, String picturePath){
		int insertRowCnt = 0;
	...
	public int insertBookmark(int userCode, int widgetCode, String title, String url){
		int insertRowCnt = 0;
		int randomNum = -1;
	...
	public int insertEmailInfo(int userCode, int widgetCode, int emailType, String emailId, String emailPw){
		int insertRowCnt = 0;
		...
		try
		{
			...
			ps.setInt(1,userCode);
			ps.setInt(2, widgetCode);
			ps.setInt(3,emailType);
			ps.setString(4, emailId);
			ps.setString(5, emailPw);
	...
	public int insertSchedule(int userCode, int widgetCode, String date, String title, int flag){
		int insertRowCnt = 0;
	...
	public int updateWidgetInfo(WidgetInfo widget){
		...
		try
		{
			...
			ps.setDouble(1,widget.getWidgetXPos());
			ps.setDouble(2,widget.getWidgetYPos());
			ps.setDouble(3,widget.getWidgetXSize());
			ps.setDouble(4,widget.getWidgetYSize());
	...
	public int selectWidgetCodeByUserCodeWidgetType(int widgetCode, int widgetType){
		int tempWidgetCode = -1;
		PreparedStatement ps=null;
		
		try
		{
			String sql="SELECT * FROM widget_tb WHERE user_code=? AND widget_type=?";
			ps=super.getConn().prepareStatement(sql);
			ps.setInt(1, widgetCode);
			ps.setInt(2, widgetType);
			ResultSet rs=ps.executeQuery();
			while(rs.next()){
				tempWidgetCode = rs.getInt("widget_code");
			}
	...
	public WidgetInfo selectWidgetInfoByWidgetCode(int widgetCode){
		...
		try
		{
			...
			while(rs.next()){
				...
				double widgetXPos = rs.getDouble("widget_x_pos");
				double widgetYPos = rs.getDouble("widget_y_pos");
				double widgetXSize = rs.getDouble("widget_x_size");
				double widgetYSize = rs.getDouble("widget_y_size");
	...
	public ArrayList<WidgetInfo> selectWidgetInfoByUserCode(int userCode){
		...
		try
		{
			...
			while(rs.next()){
				...
				WidgetInfo tempWidgetInfo = new WidgetInfo(rs.getInt("widget_code"), rs.getString("widget_id"), rs.getInt("widget_type"), rs.getDouble("widget_x_pos"), rs.getDouble("widget_y_pos"), rs.getDouble("widget_x_size"), rs.getDouble("widget_y_size"));	
	...
	public int deletePictureInfoByPictureCode(int userCode, int pictureCode){
		...
		try
		{
			String sql="DELETE FROM picture_tb WHERE user_code=? AND picture_code=?";
			ps=super.getConn().prepareStatement(sql);
			ps.setInt(1, userCode);
			ps.setInt(2, pictureCode);
